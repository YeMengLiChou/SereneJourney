...
sealed class Res <out T> {
    ...
    /** 请求成功时 */
    data class Success<out R>(val value: R) : Res<R>()
     ...
     * ```
     * */
    inline fun onSuccess(crossinline event: (value: T) -> Unit): Res<T> {
        if (this is Success) {
            // 此时已经自动转为 Success 类型
     ...
     * ```
     * */
    inline fun onError(event: (e: Throwable) -> Unit): Res<T> {
        if (this is Error) {
            // 此时已经自动转为 Error 类型
     ...
     * ```
     * */
    inline fun onRetry(event: (attempt: Int) -> Unit): Res<T> {
        if (this is Retry) {
            // 此时已经自动转为 Retry 类型
